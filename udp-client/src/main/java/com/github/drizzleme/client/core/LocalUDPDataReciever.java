...
import com.github.drizzleme.core.bo.*;
...
public class LocalUDPDataReciever {
    ...
    public void startup()
    {
                ...
                public void run()
                {
                    try
                    {
                        if (ClientCoreSDK.DEBUG) {
                            d(LocalUDPDataReciever.TAG, "【IMCORE】本地UDP端口侦听中，端口=" + ConfigEntity.localUDPPort + "...");
                        }
                    ...
                    {
                        w(LocalUDPDataReciever.TAG, "【IMCORE】本地UDP监听停止了(socket被关闭了?)," + eee.getMessage(), eee);
                    }
                }
            });
            this.thread.start();
        ...
        {
            w(TAG, "【IMCORE】本地UDPSocket监听开启时发生异常," + e.getMessage(), e);
        }
    }
        ...
        public void handleMessage(DatagramPacket p)
        {
            ...
            try
            {
                ...
                if (pFromServer.isQoS())
                {
                    ...
                    {
                        if (ClientCoreSDK.DEBUG) {
                            d(LocalUDPDataReciever.TAG, "【IMCORE】【QoS机制】" + pFromServer.getFp() + "已经存在于发送列表中，这是重复包，通知应用层收到该包罗！");
                        }
                ...
                switch (pFromServer.getType())
                {
                    ...
                    {
                        if (ClientCoreSDK.DEBUG) {
                            p(LocalUDPDataReciever.TAG, "【IMCORE】收到服务端回过来的Keep Alive心跳响应包.");
                        }
                    ...
                    case ProtocalType.C.FROM_CLIENT_TYPE_OF_RECIVED:
                    {
                        ...
                        if (ClientCoreSDK.DEBUG) {
                            i(LocalUDPDataReciever.TAG, "【IMCORE】【QoS】收到" + pFromServer.getFrom() + "发过来的指纹为" + theFingerPrint + "的应答包.");
                        }
                    ...
                    case ProtocalType.S.FROM_SERVER_TYPE_OF_RESPONSE$FOR$ERROR:
                    {
                        ...
                        if (errorRes.getErrorCode() == 301)
                        {
                            ...
                            e(LocalUDPDataReciever.TAG, "【IMCORE】收到服务端的“尚未登陆”的错误消息，心跳线程将停止，请应用层重新登陆.");
                    ...
                    default:
                        w(LocalUDPDataReciever.TAG, "【IMCORE】收到的服务端消息类型：" + pFromServer.getType() + "，但目前该类型客户端不支持解析和处理！");
                }
            }
            catch (Exception e)
            {
                w(LocalUDPDataReciever.TAG, "【IMCORE】处理消息的过程中发生了错误.", e);
            }
        }
        ...
        private void sendRecievedBack(final Protocal pFromServer)
        {
            if(pFromServer.getFp() != null)
            {
                new LocalUDPDataSender.SendCommonDataAsync(
                    ...
                    {
                        if(ClientCoreSDK.DEBUG)
                            d(TAG, "【IMCORE】【QoS】向"+pFromServer.getFrom()+"发送"+pFromServer.getFp()+"包的应答包成功,from="+pFromServer.getTo()+"！");
                    }
                }.execute();
            }
            else
            {
                w(TAG, "【IMCORE】【QoS】收到"+pFromServer.getFrom()+"发过来需要QoS的包，但它的指纹码却为null！无法发应答包！");
            }
        }
    }
}
